...
struct DECLSPEC_NOVTABLE IWeakRef : IUnknown
{
	...
	virtual HRESULT STDMETHODCALLTYPE Resolve (REFIID riid, void** objectReference) = 0;

	template <typename T>
	HRESULT Resolve (T** objectReference)
	{
...
class WeakRefToThis
{
	struct Impl : IWeakRef
	{
		...
		virtual HRESULT STDMETHODCALLTYPE Resolve (REFIID riid, void** objectReference) override
		{
			RETURN_HR_IF(E_POINTER, !objectReference);
			RETURN_HR_IF(E_FAIL, !_ptr);
			return _ptr->QueryInterface(riid, objectReference);
		}
	};

...
public:
	...
	virtual HRESULT RunStep (IBuildStepCallback* callback) override
	{
		...
		BOOL bres = CreateProcess (NULL, _cmdLine.get(), NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL,
	...
	static void CALLBACK TimerProcStatic (HWND hwnd, UINT uMsg, UINT_PTR timerId, DWORD dwTime)
	{
		...
		{
			bool complete;
			pendingSteps[i]->ReadProcessOutput(&complete);
			if (complete)
			{
	...
	HRESULT ReadProcessOutput (bool* complete)
	{
		BOOL bres;
		HRESULT hr;
		...
		*complete = false;
		...
		//DWORD dwres = WaitForSingleObject (step->_processInfo.hProcess, 0);
		//if (dwres == WAIT_OBJECT_0)
		
		//stdoutWriteHandle = nullptr;

		while(ReadFile (_pipeHandle.get(), buffer, sizeof(buffer), nullptr, &_overlapped))
		{
			DWORD bytesRead;
			bres = GetOverlappedResult (_pipeHandle.get(), &_overlapped, &bytesRead, FALSE); RETURN_LAST_ERROR_IF(!bres);

			char str[20];
			sprintf_s(str, "read=%u ", bytesRead);
			OutputDebugStringA(str);

			// Looking at its source code, sjasmplus seems to be ASCII-only, but let's assume UTF-8.
			for (uint32_t i = 0; i < bytesRead; i++)
			{
				...
				else if (buffer[i] == 0x0A)
				{
					...
					hr = MakeBstrFromString (_lineBuffer.begin() /*+ _outputWindowCount*/, _lineBuffer.end(), &bufferU); RETURN_IF_FAILED(hr);
					if (_lineParser)
					{
		...
		if (gle == ERROR_IO_PENDING)
		{
			...
			{
				// Add a newline for the sake of readability.
				bool pushed = _lineBuffer.try_push_back({ 0x0D, 0x0A }); RETURN_HR_IF(E_OUTOFMEMORY, !pushed);
				wil::unique_bstr bufferU;
		...
		if (gle == ERROR_BROKEN_PIPE)
		{
			...
			DWORD exitCode = -1;
			bres = GetExitCodeProcess (_processInfo.hProcess, &exitCode); RETURN_IF_WIN32_BOOL_FALSE(bres);

			if (_lineBuffer.size())
			{
				// There's some output for which we are waiting a CR/LF, but the process exited.
				// Let's send the output now.
				wil::unique_bstr str;
				hr = MakeBstrFromString(_lineBuffer.begin(), _lineBuffer.end(), &str); RETURN_IF_FAILED(hr);
			...
			if (exitCode)
			{
				...
				if (_printExitCode)
				{
					...
					auto message = wil::make_cotaskmem_string_nothrow(nullptr, bufferSize); RETURN_IF_NULL_ALLOC(message);
					swprintf_s(message.get(), bufferSize + 1, Format, exitCode, _cmdLine.get());
					auto hr = _outputWindow->OutputTaskItemStringEx2 (message.get(), (VSTASKPRIORITY)0, (VSTASKCATEGORY)0,
						nullptr, 0, nullptr, 0, 0, _projectName.get(), nullptr, nullptr); RETURN_IF_FAILED(hr);
				}
			}
			else
